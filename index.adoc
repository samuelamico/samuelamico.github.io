...
[source,python,linenums,highlight=2..5]
----
...
== Motion Detector
Continuing to use the idea of image histogram, in this example we'll detect if a moviment occurs in a ROI. The main idea of the code is based on the use of correlation between two frames of an video. However, we'll use only the Blue Channel on openCV (you can use the Red or Green) to use the  HistComparation and calcHist on the code.
...
[.left]
.Motion image
image::Motion.png[Motion,900,800]

[.right]
.DectMotion image
image::DectMotion.png[DectMotion,900,800]
...
[.left]
.ImgOrig image
image::ImgOrig.png[ImgOrig,900,800]

[.right]
.FiltroLaplaceGauss image
image::FiltroLaplaceGauss.png[FiltroLaplaceGauss,900,800]
